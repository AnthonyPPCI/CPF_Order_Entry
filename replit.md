...
-   **Differential Markup**: Standalone components (acrylic, backing, mats) use configurable flat markup (`standaloneComponentMarkup`, default 3.5×) when ordered without a frame. Components ordered with frames use configurable flat markup (`frameComponentMarkup`, default 3.0×), incentivizing complete orders. Both values are adjustable through the control panel.
...
-   **Tiered Markup System**: Implements cost-based markup tiers that automatically adjust pricing - lower-cost items get higher markups (ensuring profitability on small orders) while higher-cost items get lower markups (staying competitive on large orders). Global markup lever of 3.2× scales all tiers proportionally. Minimum price floor of $29 applies only to orders with frames (not standalone component orders), scales all components proportionally when triggered. Markup tiers: $0-$10 (5.12×), $10-$50 (3.84×), $50-$150 (3.2×), $150-$300 (2.72×), $300+ (2.24×). Changed from flat 2.75× markup on October 24, 2025.
