...
-   **FrameBox Centralized Authentication**: Integrated with the main FrameBox platform's authentication system using shared PostgreSQL session storage. Users authenticate via framesbox.com and sessions are shared across all apps via express-session and connect-pg-simple. Permission system uses user roles and accessible app arrays to control access - users must have 'order_entry' in their accessibleApps array (or be super_admin/admin) to access this application. Session cookies use domain `.framesbox.com` in production with 7-day TTL. All API routes protected with authentication middleware except public webhooks.
